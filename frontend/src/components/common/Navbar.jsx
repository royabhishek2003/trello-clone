...
export const Navbar = ({ onMenuClick, title }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-14 border-b shadow-sm bg-navbar/80 backdrop-blur-xl flex items-center px-4 justify-between z-50 transition-colors duration-300">
      <div className="flex items-center gap-x-4">
        ...
        <div className="hidden md:flex items-center gap-x-2">
          <Button variant="ghost" size="sm" onClick={() => navigate('/')} className="hover:bg-hover-bg flex items-center gap-2 px-2">
            <img src="/logo.svg" alt="Logo" className="w-5 h-5" />
            <span className="font-bold text-lg text-foreground pb-1">Taskify</span>
          </Button>
          <CreateBoardPopover side="bottom" sideOffset={18}>
            <Button variant="default" size="sm" className="hidden md:block bg-sky-700 hover:bg-sky-800 text-white h-auto py-1.5 px-3">
              Create
            </Button>
          </CreateBoardPopover>
        </div>
        ...
        {/* Mobile Title */}
        {title && (
          <div className="md:hidden font-bold text-lg text-foreground truncate max-w-[120px] shrink min-w-0">
            {title}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-x-2 shrink-0">
          ...
          {organizations && organizations.length > 0 ? (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="flex items-center gap-x-2 h-auto py-1 px-2 border-none hover:bg-hover-bg">
                  {currentOrg ? (
                    <>
                      ...
                      <span className="font-medium text-sm text-foreground">{currentOrg.name}</span>
                    </>
                  ) : (
                    <span className="font-medium text-sm text-foreground">Select Workspace</span>
                  )}
                ...
                <div className="flex flex-col">
                  {organizations.map(org => (
                    <Button 
                      key={org._id} 
                      variant="ghost" 
                      className="justify-start rounded-none h-auto py-2 hover:bg-hover-bg"
                      onClick={() => {
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" className="rounded-full w-8 h-8 p-0 shrink-0 hover:bg-hover-bg">
          ...
          <PopoverContent className="w-40 p-2">
            <div className="flex flex-col gap-y-2">
              ...
              <Button variant="ghost" size="sm" onClick={handleLogout} className="w-full text-red-600 hover:text-red-700 hover:bg-red-500/10">
                Log Out
              </Button>
            </div>
          </PopoverContent>
