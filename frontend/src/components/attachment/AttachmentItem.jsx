...
const getFileIcon = (fileType) => {
  switch (fileType) {
    ...
    case 'link': return <div className="bg-muted/50 p-1.5 rounded"><LinkIcon className="h-5 w-5 text-muted-foreground" /></div>;
...
export const AttachmentItem = memo(({ attachment, onDelete, onRename, onPreview, isCover, onSetCover, onRemoveCover, onComment }) => {
  ...
  return (
    <div className="flex items-center gap-x-4 mb-3 hover:bg-hover-bg p-2 rounded-md transition-colors group">
      {/* Thumbnail or Icon */}
      <div 
        className={`${attachment.fileType === 'link' ? 'w-[48px] h-[48px] bg-input' : 'w-[112px] h-[80px] bg-muted'} rounded-md shrink-0 flex items-center justify-center overflow-hidden cursor-pointer relative`}
      ...
      <div className="flex flex-col flex-1 min-w-0 py-1">
        {!isEditing ? (
          <p 
            className="font-bold text-foreground text-sm truncate cursor-pointer hover:underline mb-1"
            onClick={() => attachment.isImage ? onPreview(attachment.fileUrl) : window.open(attachment.fileUrl, '_blank')}
          >
            {attachment.fileName}
          </p>
          ...
          <div className="flex flex-col gap-y-2 mb-2">
            ...
            {attachment.fileType === 'link' && (
              <Input
                ...
                className="h-8 text-sm px-2 w-full max-w-[200px] bg-input text-foreground border-input focus:border-primary"
              />
            )}
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-x-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
            <Button 
              variant="ghost" 
              size="icon"
              className="h-7 w-7 text-muted-foreground hover:text-foreground hover:bg-hover-bg"
            ...
            <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
              <PopoverTrigger asChild>
                <Button 
                  variant="ghost" 
                  size="icon"
                  className="h-7 w-7 text-muted-foreground hover:text-foreground bg-muted hover:bg-hover-bg"
                >
                  <MoreHorizontal className="h-4 w-4" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1 bg-popover border-border text-popover-foreground" align="end">
                <div className="flex flex-col">
                  <Button variant="ghost" className="justify-start h-8 px-2 hover:bg-hover-bg hover:text-foreground rounded-sm text-sm font-normal" onClick={() => handleAction('edit')}>
                    Edit
                  </Button>
                  <Button variant="ghost" className="justify-start h-8 px-2 hover:bg-hover-bg hover:text-foreground rounded-sm text-sm font-normal" onClick={() => handleAction('comment')}>
                    Comment
                  </Button>
                  {attachment.fileType !== 'link' && (
                    <>
                      <Button variant="ghost" className="justify-start h-8 px-2 hover:bg-hover-bg hover:text-foreground rounded-sm text-sm font-normal" onClick={() => handleAction('download')}>
                        Download
                      </Button>
                      {attachment.isImage && (
                        <Button variant="ghost" className="justify-start h-8 px-2 hover:bg-hover-bg hover:text-foreground rounded-sm text-sm font-normal" onClick={() => handleAction('cover')}>
                          {isCover ? 'Remove cover' : 'Make cover'}
                        </Button>
                      )}
                    </>
                  )}
                  <Button variant="ghost" className="justify-start h-8 px-2 text-destructive hover:bg-destructive/20 hover:text-destructive rounded-sm text-sm font-normal" onClick={() => handleAction('remove')}>
                    Remove
                  </Button>
                </div>
              </PopoverContent>
